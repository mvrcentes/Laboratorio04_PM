...
void *soldReport(void *arg){
    ...
    ps->totalSold += ps->products.at(ps->month).at(ps->id).getProductSales();
...
void *varCostReport(void *arg){
    ...
    ps->varCost = ps->products.at(ps->month).at(ps->id).getUnitCost();
...
void *varUtilityReport(void *arg){
    Parameters *ps; 
    ps = (Parameters *)arg;
    ps->monthUtility += ps->products.at(ps->month).at(ps->id).getUtility();
    pthread_exit(NULL);
};
...
int main(){
    ...
    while (!salir){
        ...
        if (mes == 1 || mes == 2){
            ...
            for (long i=0; i < TOTAL_PRODUCTS; i++){
                p.id = i;
                std::cout << p.products.at(p.month).at(p.id).getNombre() << ":\tQ" << (p.products.at(p.month).at(p.id).getProductSales()) << std::endl;
                pthread_create(&tid[i], NULL, soldReport, ( void *)&p);
            }
            
            printf ("\n --------------------------------------------- \n");
            printf ("Utilidad por producto \n\n");
            for (long k=0; k < TOTAL_PRODUCTS; k++){
                p.id = k;
                std::cout << p.products.at(p.month).at(p.id).getNombre() << ":\tQ" << (p.products.at(p.month).at(p.id).getUtility()) << std::endl;
                pthread_create(&tid[k], NULL, varCostReport, ( void *)&p);
                pthread_create(&tid[k], NULL, varUtilityReport, ( void *)&p);
            }
            
            for(long j=0; j < TOTAL_PRODUCTS; j++){
            ...
            std::cout << "Total de venta: \t" << p.totalSold << std::endl;
            std::cout << "Total de costos variables: \t" <<  varCostmonth  << std::endl;
            std::cout << "Total de utilidad: \t" << p.monthUtility  - varCost<< std::endl;
            printf ("\n --------------------------------------------- \n\n\n\n");
        } else if (mes == 3){
            salir = true;
        } else {
